...
@section('page-title', 'Catatan Pengeluaran (Kas Keluar)')
@section('breadcrumb', 'Admin / Keuangan / Pengeluaran')
...
<div class="row g-4">
    ...
    <div class="col-lg-4">
        <div class="card-admin">
            <div class="card-header">
                <h6 class="mb-0 fw-bold" id="formTitle"><i class="bi bi-arrow-up-right-circle me-2 text-danger"></i>Pencatatan Uang Keluar</h6>
            </div>
            <div class="card-body">
                <form id="pengeluaranForm" method="POST" action="{{ route('admin.keuangan.pengeluaran.store') }}" enctype="multipart/form-data">
                    @csrf
                    ...
                    <div class="mb-3">
                        <label class="form-label-admin">Kategori Pengeluaran <span class="text-danger">*</span></label>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea name="keterangan" id="transaksiKeterangan" class="form-control form-control-admin" rows="3" placeholder="Pembelian ATK, konsumsi rapat..."></textarea>
                    </div>

                    <div class="mb-4">
                        <label class="form-label-admin">Bukti Transaksi <small class="text-muted">(Foto/Kuitansi)</small></label>
    ...
    <div class="col-lg-8">
        <div class="card-admin">
            <div class="card-header">
                <h6 class="mb-0 fw-bold"><i class="bi bi-list-task text-primary me-2"></i>Daftar Pengeluaran</h6>
            </div>
            <div class="card-body p-0">
                <table class="table table-admin mb-0">
                        ...
                        <td class="text-center">
                            <div class="d-flex justify-content-center gap-2">
                                <button type="button" class="btn-edit" 
                                    onclick="editPengeluaran({{ $p->id }}, '{{ $p->tanggal->format('Y-m-d') }}', {{ $p->kas_id }}, {{ $p->kategori_id }}, {{ $p->jumlah }}, '{{ $p->keterangan }}', '{{ $p->bukti_foto ? Storage::url($p->bukti_foto) : '' }}', '{{ route('admin.keuangan.pengeluaran.update', $p) }}')">
                                    <i class="bi bi-pencil"></i>
                                </button>
                                <form method="POST" action="{{ route('admin.keuangan.pengeluaran.destroy', $p) }}" onsubmit="return confirm('Hapus catatan pengeluaran ini? Tindakan ini akan mengembalikan saldo kas terkait.')">
                                    @csrf @method('DELETE')
                    ...
                    @empty
                    <tr>
                        <td colspan="6" class="text-center py-4 text-muted">Belum ada pengeluaran dicatat.</td>
                    </tr>
                    @endforelse
                    </tbody>
                </table>
...
@push('scripts')
<script>
function editPengeluaran(id, tanggal, kasId, kategoriId, jumlah, keterangan, buktiUrl, url) {
    document.getElementById('formTitle').innerHTML = '<i class="bi bi-pencil-fill me-2 text-warning"></i>Edit Uang Keluar';
...
function resetForm() {
    document.getElementById('formTitle').innerHTML = '<i class="bi bi-arrow-up-right-circle me-2 text-danger"></i>Pencatatan Uang Keluar';
    ...
    const form = document.getElementById('pengeluaranForm');
    form.action = "{{ route('admin.keuangan.pengeluaran.store') }}";
